...
void PartitionAlgo(
  ...
  for (int j = leftPos; j < rightPos; j++ ) {
    ...
    } else if (sequence[j] < sequence[pivotIdx]) {
        pivotEnd += 1;
    }
  }
...
void QuickSort(
  ...
  if (!sequence.empty() && leftPos != rightPos) {
    int pivotIdx0, pivotIdx1;
    PartitionAlgo(sequence, leftPos, rightPos, pivotIdx0, pivotIdx1);
    QuickSort(
      sequence, leftPos, pivotIdx0 != leftPos ? pivotIdx0 - 1 : pivotIdx0);
    QuickSort(
      sequence, pivotIdx1 != rightPos ? pivotIdx1 + 1 : pivotIdx1, rightPos);
  }
  return;
}
